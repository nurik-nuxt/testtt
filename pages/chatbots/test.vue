                    ...
                    <div class="mt-3">
                      ...
                      >
                        ...
                        <i
                            class="pi pi-trash ml-auto"
                            style="cursor: pointer; color: #EE9186; font-size: 18px"
                            @click="deleteCustomApiParameter(index, paramIndex, 'params')"
                        ></i>
                      </div>
                      <Button
                          class="mt-3 text-center"
                          @click="addCustomApiParameter(index, 'params')"
                    ...
                    <div class="mt-3">
                      ...
                      >
                        ...
                        <i
                            class="pi pi-trash ml-auto"
                            style="cursor: pointer; color: #EE9186; font-size: 18px"
                            @click="deleteCustomApiParameter(index, bodyIndex, 'body')"
                        ></i>
                      </div>
                      <Button
                          class="mt-3 text-center"
                          @click="addCustomApiParameter(index, 'body')"
                          :label="`+ ${t('newParameter')}`"
                      ></Button>
                    </div>
                  </div>
...
const parameterActions = ref([
  ...
  { title: 'Параметр из функции', value: 'parameterFunction' },
  { title: 'Данные пользователя', value: 'userData' },
]);
    ...
    set(value: string) {
      ...
      if (!action) {
        ...
        action = {
          ...
          parameters: { method: value, url: '', params: [], headers: [], body: [] },
        };
        props.botFunctions[index].actions.push(action);
      } else {
        // Update the existing action
  ...
  return computed({
    ...
    set(value: string) {
      ...
      if (!action) {
        ...
        action = {
          name: 'send_to_webhook',
          parameters: { method: '', url: value, params: [], headers: [], body: [] },
        };
        props.botFunctions[index].actions.push(action);
      } else {
        // Update the existing action
  ...
  return computed({
    ...
    set(value: any[]) {
      ...
      if (!action) {
        ...
        action = {
          name: 'send_to_webhook',
          parameters: { method: '', url: '', params: value, headers: [], body: [] },
        };
        props.botFunctions[index].actions.push(action);
      } else {
        // Update the existing action
  ...
  return computed({
    ...
    set(value: any[]) {
      ...
      if (!action) {
        ...
        action = {
          name: 'send_to_webhook',
          parameters: { method: '', url: '', params: [], headers: value, body: [] },
        };
        props.botFunctions[index].actions.push(action);
      } else {
        // Update the existing action
  ...
  return computed({
    ...
    set(value: any[]) {
      ...
      if (!action) {
        ...
        action = {
          name: 'send_to_webhook',
          parameters: { method: '', url: '', params: [], headers: [], body: value },
        };
        props.botFunctions[index].actions.push(action);
      } else {
        // Update the existing action
...
const addCustomApiParameter = (functionIndex: number, paramType: 'params' | 'headers' | 'body') => {
  ...
  if (!action) {
    action = {
      name: 'send_to_webhook',
      parameters: { method: '', url: '', params: [], headers: [], body: [] },
    };
