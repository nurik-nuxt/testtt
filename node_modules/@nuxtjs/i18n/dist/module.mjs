...
function prefixLocalizedRoute(localizeOptions, options, extra = false) {
  ...
  return !extra && !isChildWithRelativePath && // skip default locale if strategy is 'prefix_except_default'
...
const mergeLayerLocales = (options, nuxt) => {
  ...
  const localeObjects = options.locales.filter((x) => x !== "string");
  const absoluteConfigMap = /* @__PURE__ */ new Map();
  ...
  for (const absoluteLocaleObject of absoluteLocaleObjects) {
    ...
    if (absoluteConfigMap.has(langDir)) {
      ...
      absoluteConfigMap.set(langDir, {
        langDir,
        projectLangDir,
        locales: [...entry.locales, absoluteLocaleObject]
      });
      continue;
    }
    absoluteConfigMap.set(langDir, {
...
async function resolveLayerVueI18nConfigInfo(options, nuxt, buildDir) {
  const logger = useLogger(NUXT_I18N_MODULE_ID);
  const layers = [...nuxt.options._layers];
